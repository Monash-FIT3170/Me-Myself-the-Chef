...
const GENERATE_RECIPE = 'Resend the previous recipe with the following JSON format ensuring that float are limited to 2 decimal places and no fractions are used, additionally use standard abbreviated measures where possible and measure spices with "g". Ensure that the nutrients - Fat, Calories, Saturated Fat, Carbohydrates, Sugar, Sodium are included in the nutrients section and their quanitity is for the whole recipe, not per serving:{"title": string, "image": "image_link", "servings": int, "extendedIngredients": [ {"nameClean": string, "amount": float,  "measures": { "metric": { "amount": int, "unitShort": string }}}],"analyzedInstructions": [{"steps": [{"number": int, "step": string}]}], "nutrition": { "nutrients": [{"name": string, "amount": float, "unit": string}]}, "readyInMinutes": int, "preperationMinutes": int}';
